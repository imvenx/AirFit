<template>
  <div class="hdmi-test">
    ...
    <div class="hud">
      ...
      <div class="hint">
        Calibrate 4 corners on the camera view, then touch the screen.
      </div>
      <div class="buttons">
...
function resizeScreenCanvas() {
  ...
  // If we have 4 source corners, recompute homography w.r.t. new canvas size
...
function drawMain() {
  ...
  if (filtered) {
    ...
    ctx.beginPath()
    ctx.arc(filtered.x, filtered.y, 18, 0, Math.PI * 2)
    ctx.fill()
    ctx.restore()
  }
}
...
function solveLinearSystem(A: number[][], b: number[]): number[] | null {
  // Gaussian elimination for an n x n system
  const n = A.length
  if (n === 0) return null
  const m = A[0].length
  ...
  // Extract solution last column; assuming full rank
...
onMounted(async () => {
  ...
  resizeScreenCanvas()
  // Size mini overlay to video box soon after layout
...
watch(landmarks, () => {
  const video = videoRef.value
  if (!video) return
  // Use first detected hand's index fingertip
  const hands = landmarks.value
  if (!hands || hands.length === 0) { drawMain(); return }
  const lm = hands[0]
  const ptCam = { x: lm[8].x * video.videoWidth, y: lm[8].y * video.videoHeight }
  const mapped = applyH(ptCam)
  if (mapped) {
    if (!filtered) filtered = { ...mapped }
    else {
      filtered.x += SMOOTH_ALPHA * (mapped.x - filtered.x)
      filtered.y += SMOOTH_ALPHA * (mapped.y - filtered.y)
    }
  }
  drawMain()
})
